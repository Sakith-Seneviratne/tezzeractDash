...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/setup?error=linkedin_auth_failed`);
  }
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        config: {
          ...
          user_id: userData.id,
          user_name: `${userData.localizedFirstName} ${userData.localizedLastName}`,
        },
        status: 'active',
      });

    if (insertError) {
      console.error('Error saving LinkedIn integration:', insertError);
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/setup?error=save_failed`);
    }

    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/setup?success=linkedin_connected`);
  } catch (error) {
